...
/**
crypto_(secret)box_easy 인터페이스

원래대로라면 crypto_(secret)box는 앞에 0으로 패딩이 붙어 있어야 하는데,
그럼 m -> padded m이랑 c -> 루아 문자열, 즉 두 차례 메모리 복사가 일어나야 한다.
그걸 피하기 위해 패딩 없는 입력을 받아 패딩이 있는 것처럼 흉내내어 함수를 특수화한 버전.
secretbox를 위한 함수긴 하지만 어차피 구현상 box_afternm = secretbox라 하나만 있어도 된다.

아래 코드는 lua-nacl에서 가져 온 것. TweetNaCl과 동일하게 public domain이다.
Sodium에도 비슷한 게 있는데, lua-nacl의 버전은 TweetNaCl 함수만 써서 재구현한 것이다.
*/

int crypto_stream_salsa20_xor_skip(u8 *c, const u8 *m, u64 b, const u8 *n, const u8 *k, u64 skip)
{
	u8 z[16], x[64];
	u64 u, i;
	if (!b) return 0;
...
int crypto_secretbox_easy(u8 *c, const u8 *m, u64 d, const u8 *n, const u8 *k)
{
	u8 s[32], x[32];
	crypto_core_hsalsa20(s, n, k, sigma);
...
int crypto_secretbox_easy_open(u8 *m, const u8 *c, u64 d, const u8 *n, const u8 *k)
{
	u8 s[32], x[32];
	int v;
	if (d < 16) return -1;
...
int crypto_box_easy_afternm(u8 *c, const u8 *m, u64 d, const u8 *n, const u8 *k)
{
	return crypto_secretbox_easy(c, m, d, n, k);
}

int crypto_box_easy_open_afternm(u8 *m, const u8 *c, u64 d, const u8 *n, const u8 *k)
{
	return crypto_secretbox_easy_open(m, c, d, n, k);
}

int crypto_box_easy(u8 *c, const u8 *m, u64 d, const u8 *n, const u8 *y, const u8 *x)
{
	u8 k[32];
	crypto_box_beforenm(k, y, x);
	return crypto_box_easy_afternm(c, m, d, n, k);
}

int crypto_box_easy_open(u8 *m, const u8 *c, u64 d, const u8 *n, const u8 *y, const u8 *x)
{
	u8 k[32];
	crypto_box_beforenm(k, y, x);
	return crypto_box_easy_open_afternm(m, c, d, n, k);
}
